    ...
    fun minify(originalTz: VTimeZone, startTemporal: Temporal?): VTimeZone {
        ...
        for (observance in originalTz.observances) {
            ...
            if (latest == null)         // observance begins after "start", keep in any case
                ...
                when (observance) {
                    is Standard ->
                        if (latestStandard == null || TemporalAdapterCompat.isAfter(latest, latestStandard.first))
                            latestStandard = Pair(latest, observance)
                    is Daylight ->
                        if (latestDaylight == null || TemporalAdapterCompat.isAfter(latest, latestDaylight.first))
        ...
        latestDaylight?.second?.let { daylight ->
            ...
            for (rDate in daylight.getProperties<RDate<Temporal>>(Property.RDATE)) {
                if (rDate.dates.any { !TemporalAdapterCompat.isBefore(it, startLocal) }) {
                    // RDATE in the future
                    keep += daylight
                    return@let
                }
