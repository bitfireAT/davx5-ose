...
class CachedGroupMembershipHandler(
    ...
    override fun forMimeType() = ContactContract.CachedGroupMembership.CONTENT_ITEM_TYPE

    override fun handle(values: ContentValues, contact: Contact) {
        super.handle(values, contact)

        if (groupMethod == GroupMethod.GROUP_VCARDS) {
            val groupId = values.getAsLong(ContactContract.CachedGroupMembership.GROUP_ID) ?: return
