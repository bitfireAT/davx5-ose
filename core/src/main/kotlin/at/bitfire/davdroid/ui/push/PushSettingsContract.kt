/*
 * Copyright © All Contributors. See LICENSE and AUTHORS in the root directory for details.
 */

package at.bitfire.davdroid.ui.push

import android.graphics.drawable.Drawable

interface PushSettingsContract {
    data class State(
        val isPushEnabled: Boolean = true,
        val pushCollectionsMessage: PushCollectionsMessage = PushCollectionsMessage.Disabled,
        val selectedPushDistributor: String? = null,
        val defaultPushDistributor: String? = null,
        val pushDistributors: List<PushDistributorInfo> = emptyList(),
        val showSelectDefaultPushDistributorCard: Boolean = false,
        val showAdvertiseUnifiedPushCard: Boolean = false
    )

    enum class PushCollectionsMessage {
        Disabled,
        NonePushCapable,
        SomePushCapable
    }

    data class PushDistributorInfo(
        val packageName: String,
        val appName: String,
        val appIcon: Drawable?,
    )
}