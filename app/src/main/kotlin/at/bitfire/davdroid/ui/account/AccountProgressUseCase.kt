...
class AccountProgressUseCase @Inject constructor(
    ...
    operator fun invoke(
        ...
        dataTypesFlow: Flow<List<SyncDataType>>         // FIXME flow not required anymore because SyncDataType.TASKS always exists
    ): Flow<AccountProgress> {
        val serviceRefreshing = isServiceRefreshing(serviceFlow)
        val syncPending = isSyncPending(account, dataTypesFlow)
        val syncRunning = isSyncRunning(account, dataTypesFlow)
    ...
    @OptIn(ExperimentalCoroutinesApi::class)
    fun isSyncPending(account: Account, dataTypesFlow: Flow<List<SyncDataType>>): Flow<Boolean> =
        dataTypesFlow.flatMapLatest { dataTypes ->
    ...
    @OptIn(ExperimentalCoroutinesApi::class)
    fun isSyncRunning(account: Account, dataTypesFlow: Flow<List<SyncDataType>>): Flow<Boolean> =
        dataTypesFlow.flatMapLatest { dataTypes ->
            syncWorkerManager.hasAnyFlow(
